...
const getSrkGrowProfile: AppRouteImplementationOrOptions<typeof growContract.getSrkGrowProfile> = async ({ params }) => {
    try {
        const profileExist = await growSocialMediaPackageUserModel.findOne({ _id: params.id })
