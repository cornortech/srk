...
import { useIsMobileView } from '@srk/shared/hooks';
...
export default function SendMoneyPage() {
  ...
  const {
    ...
    getValues,
    reset,
  } = useForm<TCreatePaymentIntentForm>({
  ...
  const { setSendMoneyDetails, receiverAccountName, receiverAccountNumber } =
    useSendMoneyStore();
  ...
  const { mutate: createPaymentIntentMutation } = useMutation({
    ...
    onSuccess: (data: AxiosResponse<{ paymentIntentId: string }>) => {
      ...
      navigate('/dashboard/send-money/preview');
    },
    onError: (error: any) => {
      // Handle error
        ...
        <Card className="bg-[#1a1a1a] border border-[#b68938]/40 shadow-2xl shadow-[#b68938]/10 rounded-2xl">
          <CardHeader className="flex flex-col items-start gap-2 sm:gap-3">
            <div className="flex items-center gap-2">
              ...
              {isFromQR && (
                <QrCode className="w-4 h-4" style={{ color: '#b68938' }} />
              )}
            </div>
