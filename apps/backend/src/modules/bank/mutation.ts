...
> = async ({ req, res }) => {
  try {
    const { userId } = req.params;
    const { amount, receiverAccountNumber, description } = req.body;
    ...
    const paymentIntent = await PaymentIntentModel.create({
      senderBankId: senderUserExist.srkBankId._id.toString(),
      amount,
      receiverBankId: receiverSrkBankExist._id.toString(),
