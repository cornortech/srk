  ...
  return (
    <div style={{ 
      ...
      <main>
        {/* User Info Card */}
        <div style={{ 
          background: 'linear-gradient(135deg, rgba(182, 137, 56, 0.2), rgba(182, 137, 56, 0.05))', 
          ...
          <div>
            <h2 style={{ fontSize: '1.5rem', marginBottom: '0.3rem', color: '#fff' }}>
              {user?.firstName && user?.lastName 
                ? `${user.firstName} ${user.lastName}` 
                : user?.firstName || 'User'}
            </h2>
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1rem', marginBottom: '0.3rem' }}>
              📧 {user?.email || 'No email'}
            </p>
        ...
        <div style={{ 
          ...
          <p style={{ color: 'rgba(255,255,255,0.7)', lineHeight: '1.6' }}>
            You have been successfully authenticated via SSO from SRK University.
            This page demonstrates the multi-domain Single Sign-On working correctly.
          </p>
        </div>
...
const TaskCard = ({ title, count, description }: { title: string; count: number; description: string }) => (
  <div style={{
    ...
    <p style={{ fontSize: '2.5rem', fontWeight: 'bold', color: '#b68938', marginBottom: '0.5rem' }}>
      {count}
    </p>
