...
> = async ({ body }) => {
    try {
        ...
        const packageExists = await growSocialMediaPackageModel.findById(enrollmentData.growSocialMediaPackageId);
        if (!packageExists) {
            return {
                status: 400,
                body: {
        ...
        const packageTypeExists = await growSocialMediaPackageTypeModel.findById(enrollmentData.growSocialMediaPackageTypeId);
        if (!packageTypeExists || String(packageTypeExists.growSocialMediaPackageId) !== enrollmentData.growSocialMediaPackageId) {
            return {
                status: 400,
                body: {
                    success: false,
        ...
        const packageSubTypeExists = await growSocialMediaPackageSubTypeModel.findById(enrollmentData.growSocialMediaPackageSubTypeId);
        if (!packageSubTypeExists || String(packageSubTypeExists.growSocialMediaPackageTypeId) !== enrollmentData.growSocialMediaPackageTypeId) {
            return {
                status: 400,
                body: {
                    success: false,
        ...
        // 5. Create enrollment
        const createSrkGrowPackageEnrollment = await growSocialMediaPackageEnrollmentModel.create({
            growSocialMediaPackageUserId: packageUser._id,
            growSocialMediaPackageId: enrollmentData.growSocialMediaPackageId,
            growSocialMediaPackageTypeId: enrollmentData.growSocialMediaPackageTypeId,
            growSocialMediaPackageSubTypeId: enrollmentData.growSocialMediaPackageSubTypeId,
...
> = async ({ body }) => {
    try {
        ...
        const activeEnrollment = await growSocialMediaPackageEnrollmentModel.findById(
            growSocialMediaPackageEnrollmentId
        );
        ...
        if (subType.taskType === "follow") {
            ...
            if (existingURLs.length + uniqueIncomingUrlRequest.length > allowedCount) {
                return {
                    status: 400,
                    body: {
                        success: false,
