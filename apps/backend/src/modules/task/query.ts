...
const getSocialTaskPackages: AppRouteImplementationOrOptions<
  typeof taskContract.getSocialTaskPackages
> = async () => {
  try {
...
const getAllSocialTaskEnrollments: AppRouteImplementationOrOptions<
  typeof taskContract.getAllSocialTaskEnrollments
> = async ({ query }) => {
  try {
    const enrollments = await SocialTaskPackageEnrollmentModel.find({
      status: query.status,
    });
...
const getSocialTaskEarning: AppRouteImplementationOrOptions<
  typeof taskContract.getSocialTaskEarning
> = async ({ params }) => {
  try {
    const earningExist = await SocialTaskEarningStatementModel.findOne({
      followedBy: params.id,
    });
