...
export default function BankRegisterPage() {
    ...
    const validateStep = (step: number) => {
        switch (step) {
            ...
            case 6:
                return formData.ppSizePhoto && formData.nationalIdCard;
            default:
                return true;
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (formData.ppSizePhoto)
            ppSizePhotoUrl = await (await uploadFile(formData.ppSizePhoto, "image", "bank")).url;

        if (formData.nationalIdCard)
            nationIdCardUrl = await (await uploadFile(formData.nationalIdCard, "image", "bank")).url;

        try {
            await bankApi.createBankDetailsApi(userDetails._id, {
                currentAddress: {
                    country: formData.currentCountry,
                    district: formData.currentDistrict,
                    municipality: formData.currentMunicipality,
                    province: formData.currentProvince,
                    street: formData.currentProvince,
                    wardNo: formData.currentWardNo,
                },
                documents: {
                    nationalIdCard: nationIdCardUrl,
                    ppSizePhoto: ppSizePhotoUrl
                },
                familyDetails: {
    ...
    const renderStep = () => {
        switch (currentStep) {
            case 1:
                return (
                    <div className="space-y-4 sm:space-y-5">
                        <h3 className="text-xl sm:text-2xl font-bold" style={{ color: '#b68938' }}>Basic Information</h3>
                        <Input
                            label="Full Name"
                            name="fullName"
            ...
            case 2:
                return (
                    <div className="space-y-4 sm:space-y-5">
                        <h3 className="text-xl sm:text-2xl font-bold" style={{ color: '#b68938' }}>Family Details</h3>
                        <Input
                            label="Father's Name"
                            name="fatherName"
                        ...
                        <Textarea
                            ...
                            onChange={(e) => setFormData({ ...formData, childrenNames: e.target.value.split(",") })}
                            classNames={{
                                inputWrapper: "bg-[#2a2520] border-[#b68938]/40 hover:border-[#b68938]/60 focus-within:border-[#b68938]",
                                label: "text-[#b68938]",
                                input: "text-white"
            ...
            case 3:
                return (
                    <div className="space-y-4 sm:space-y-5">
                        <h3 className="text-xl sm:text-2xl font-bold" style={{ color: '#b68938' }}>Permanent Address</h3>
                        <Input
                            label="Country"
            ...
            case 4:
                return (
                    <div className="space-y-4 sm:space-y-5">
                        <h3 className="text-xl sm:text-2xl font-bold" style={{ color: '#b68938' }}>Current Address</h3>
                        <label className="flex items-center gap-3 text-white cursor-pointer">
                            ...
                            />
                            <span className="text-sm sm:text-base">Same as permanent address</span>
                        </label>
                        <Input
                            label="Country"
                            name="currentCountry"
            ...
            case 5:
                return (
                    <div className="space-y-4 sm:space-y-5">
                        <h3 className="text-xl sm:text-2xl font-bold" style={{ color: '#b68938' }}>Identification Details</h3>
                        <Select
                            label="ID Type"
                            selectedKeys={formData.idType ? [formData.idType] : []}
                            onSelectionChange={(keys) => {
                                const selected = Array.from(keys)[0];
                                setFormData((prev) => ({ ...prev, idType: selected as string }));
                            }}
                            classNames={{
                                trigger: "bg-[#2a2520] border-[#b68938]/40 hover:border-[#b68938]/60 focus-within:border-[#b68938]",
                                label: "text-[#b68938]"
                            }}
                        >
            ...
            case 6:
                return (
                    <div className="space-y-4 sm:space-y-5">
                        <h3 className="text-xl sm:text-2xl font-bold" style={{ color: '#b68938' }}>Upload Documents</h3>
                        <Input
                            type="file"
    ...
    return (
        <div className="min-h-screen w-full bg-black flex items-center justify-center px-4 py-6 sm:py-8 mt-32">
            <Card className="w-full max-w-2xl bg-[#1a1a1a] border border-[#b68938]/40 shadow-2xl shadow-[#b68938]/10 rounded-2xl">
                <CardHeader className="flex flex-col items-center text-center gap-3 sm:gap-4 pt-4 sm:pt-6 pb-4 sm:pb-6">
                    ...
                    <h1 className="text-2xl sm:text-3xl font-bold" style={{ color: '#b68938' }}>
                        Create Account
                    </h1>
                ...
                <CardBody className="px-4 sm:px-6 pb-4 sm:pb-6">
                    ...
                    <div className="mt-4 sm:mt-6 text-center text-xs sm:text-sm">
                        <p className="text-gray-400">
                            ...
                            <NextLink href="/login" className="font-medium transition-colors hover:opacity-80" style={{ color: '#b68938' }}>
                                Sign in
                            </NextLink>
                        </p>
                    </div>
